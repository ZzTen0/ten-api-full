...
import static com.ten.project.constant.UserConstant.ADMIN_ROLE;
import static com.ten.project.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 盐值，混淆密码
     */
    private static final String SALT = "sAlTtEn";
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        synchronized (userAccount.intern()) {
            ...
            int attempts = 0;
            do {
                accessKey = signUtils.generateAccessKey();
                secretKey = signUtils.generateSecretKey();
                queryWrapper.clear();
                queryWrapper.eq("access_key", accessKey).or().eq("secret_key", secretKey);
                attempts++;
                if (attempts >= 3) {
                    throw new BusinessException(ErrorCode.OPERATION_ERROR, "accessKey或secretKey生成失败");
                }
            } while (userMapper.selectCount(queryWrapper) > 0);
